...
const STEPS = [
  {
    ...
    desc: "Browse unique, locally curated tours designed by passionate guides.",
  ...
  },
  {
    ...
    desc: "Ask questions, discuss trip expectations, and personalize your experience.",
  ...
  },
  {
    ...
    desc: "Securely book your chosen experience with our seamless booking system.",
  ...
  },
  {
    ...
    desc: "Explore like a local and share feedback to help future travelers.",
...
export default function HowItWork() {
  ...
  return (
    <section 
      ref={sectionRef}
      className="relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden bg-linear-to-b from-white to-blue-50/20"
    >
      {/* Background Elements */}
      <BackgroundElements />
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="flex flex-col gap-12 items-center">
          ...
          {/* Active Step Preview */}
          <ActiveStepPreview activeStep={activeStep} />
        </div>
...
function StepsTimeline({ activeStep, setActiveStep, controls }: StepsTimelineProps) {
  return (
    <motion.div
...
function StepCard({ step, index, isActive, setActiveStep, controls, isLast }: StepCardProps) {
  return (
    <motion.div
      variants={STEP_VARIANTS as {}}
      whileHover="hover"
      onClick={() => setActiveStep(index)}
      className={`relative cursor-pointer ${
        isActive ? 'lg:transform lg:translate-y-5' : ''
      }`}
    >
      {/* Step Number */}
      <StepNumber index={index} isActive={isActive} controls={controls} />
      ...
      <div className={`relative rounded-2xl p-6 border-2 ${
        ...
        {/* Icon */}
        <StepIcon icon={step.icon} color={step.color} isActive={isActive} />
        ...
        {/* Details List */}
        <StepDetailsList details={step.details} />

        {/* Active Step Indicator */}
        {isActive && <ActiveStepIndicator />}
      </div>
...
function StepNumber({ index, isActive, controls }: { index: number; isActive: boolean; controls: any }) {
  return (
    <motion.div
...
function StepIcon({ icon, color, isActive }: { icon: React.ReactNode; color: string; isActive: boolean }) {
  return (
    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 ${
      isActive ? 'bg-white shadow-xl' : 'bg-white/80 shadow-lg'
    }`}>
      <div className={`bg-linear-to-br ${color} bg-clip-text text-transparent`}>
        {icon}
      </div>
    </div>
  );
...
function StepDetailsList({ details }: { details: string[] }) {
  return (
...
function StepNavigationDots({ activeStep, setActiveStep }: { activeStep: number; setActiveStep: (step: number) => void }) {
  return (
...
function ActiveStepPreview({ activeStep }: { activeStep: number }) {
    ...
    >
      <div className="relative">
        ...
        <div className="relative bg-white rounded-3xl p-8 shadow-2xl border border-blue-100">
          <PreviewHeader activeStep={activeStep} />
          <PreviewIcon step={activeStepData} />
          <PreviewTitle title={activeStepData.title} />
          <ProgressBar progressPercentage={progressPercentage} />
          <BenefitsList details={activeStepData.details} />
          <PreviewCTA />
        </div>
...
function PreviewHeader({ activeStep }: { activeStep: number }) {
  return (
...
function PreviewIcon({ step }: { step: typeof STEPS[0] }) {
  return (
    <div className={`w-24 h-24 rounded-3xl flex items-center justify-center mb-8 mx-auto shadow-xl ${step.bgColor}`}>
      <div className={`text-4xl bg-linear-to-br ${step.color} bg-clip-text text-transparent`}>
        {step.icon}
      </div>
    </div>
  );
...
function BenefitsList({ details }: { details: string[] }) {
  return (
